...
local function logBlockedUse(reason, cost, coins)
    ...
    elseif reason == "stageapi_missing" then
        ConchBlessing.printError("Appraisal requires a compatible StageAPI for its independent room space.")
...
function M.onUseItem(_, _, _, player)
    ...
    -- contract. Its StageAPI room transition supplies the transition animation.
    return { Discharge = false, Remove = false, ShowAnim = false }
end
