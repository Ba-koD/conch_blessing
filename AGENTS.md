...
3. Core requires `scripts/conch_blessing_items.lua`; the item loader then requires `scripts/eid_language.lua`, `scripts/callback_manager.lua`, and the item behavior modules, and registers ItemData callbacks after all behavior modules have loaded but before the first game-start dispatch.
...
- Any item that calls a REPENTOGON-only API must note it in the item's `eid` block with a short fixed phrase only. Do not add reasons, API names, or fallback details in the EID.
  ...
  - Has a vanilla fallback: use `#{{Warning}} REPENTOGON 권장` (en `#{{Warning}} REPENTOGON recommended`), nothing more. Examples: `appraisal_certificate`, `void_dagger`.
